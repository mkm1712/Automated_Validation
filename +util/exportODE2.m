function [commandLine] = exportODE2(speciesNames,paramList,ODElist)
% exportODE2
...
commandLine{end+1} = sprintf('    finhib = 1 - act(x,rpar);\n');

commandLine{end+1} = sprintf('function z = OR(x,y)');
commandLine{end+1} = sprintf('%% OR logic gate');
commandLine{end+1} = sprintf('    z = 1-(1-x)*(1-y);\n');
...
commandLine{end+1} = sprintf('    w = rpar(1);');
commandLine{end+1} = sprintf('        v = real(cell2mat(varargin));');
commandLine{end+1} = sprintf('        h = (sum(v)/(nargin-1))^(nargin-2);');
commandLine{end+1} = sprintf('    if w == 0');
commandLine{end+1} = sprintf('        z = 0;');
commandLine{end+1} = sprintf('    elseif h == 0');
commandLine{end+1} = sprintf('        z = 0;'); 
commandLine{end+1} = sprintf('    else');
commandLine{end+1} = sprintf('        z = max(prod(v)/h,min(v));');
commandLine{end+1} = sprintf('    end');
