   ...
   methods (Access=private)
       ...
       function index = findChild(obj, child)           
        ...
        function createPlotArea(obj, newLocation, doResizeFigure)            
            ...
            set(obj.UiHandle, 'menubar', 'none', ...   %STD changed menubar to 'none'                
                'Toolbar', 'figure',...                %STD added 'Toolbar'
                'HandleVisibility', 'on', ...
                'resize', 'on', ...
                'units', 'pixels', ...
                'numbertitle', 'off');   
            
            %STD; delete Standard GUI menus here
           delete(findall(obj.UiHandle,'tag','Standard.SaveFigure'));  %Save button         
           delete(findall(obj.UiHandle,'tag','Standard.NewFigure')); % New Figure button
           delete(findall(obj.UiHandle,'tag','Standard.FileOpen')); % Open File button
           delete(findall(obj.UiHandle,'tag','Standard.PrintFigure')); %Print button
           delete(findall(obj.UiHandle,'tag','Exploration.Rotate')); %Rotate 3D
           delete(findall(obj.UiHandle,'tag','Exploration.Brushing')); %Brush
           delete(findall(obj.UiHandle,'tag','DataManager.Linking')); %Link
           delete(findall(obj.UiHandle,'tag','Annotation.InsertColorbar')); %Colarbar
           delete(findall(obj.UiHandle,'tag','Plottools.PlottoolsOn')); %Plot tools1
           delete(findall(obj.UiHandle,'tag','Plottools.PlottoolsOff')); %Plot tools2
           set(findall(obj.UiHandle,'tag','Standard.EditPlot'),'Separator', 'off');
           
   %Future versions of GUI toolbar tags may have their nameschanged; 
   %use  hToolbar = findall(gcf,'tag','FigureToolBar');get(findall(hToolbar),'tag');
